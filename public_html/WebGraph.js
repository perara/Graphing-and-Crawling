...
Drawing.WebGraph = function(options) {
    var options = options || {};
    ...
    this.url_map = options.url_map || {};
    this.url_rel = options.url_rel || [];
    ...
    function createGraph() {
        ...
        for (var key in that.url_map) {
            if (counter++ >= limit)
                break;
            var value = that.url_map[key];

            var url = value[0];
            var id = parseInt(value[1]);

            var node = new Node(parseInt(id));
        ...
        console.log(nodeids)
        for (var key in that.url_rel){
            var value = that.url_rel[key];

            var rel1 = value[0];
            var rel2 = value[1];


            if(contains(nodeids, rel1) && contains(nodeids, rel2))
            {

                if(graph.addEdge(nodemap[rel1], nodemap[rel2])) {
                    drawEdge(nodemap[rel1], nodemap[rel2]);
                }
